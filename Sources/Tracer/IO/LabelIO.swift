...
///   • **MetaImage** (`.mha`) — challenge-friendly image-processing mask
///   • **Label descriptor** (`.label.txt`) — name/color sidecar
///   • **Segmentation NRRD** (`.seg.nrrd`) — NRRD with segment metadata
...
public enum LabelIO {
    ...
    public enum Format: String, CaseIterable, Identifiable, Sendable {
        ...
        case segmentationNRRD = "Segmentation NRRD (.seg.nrrd)"
        case dicomSeg = "DICOM SEG"
        case dicomRTStruct = "DICOM RTSTRUCT"
        case labelDescriptor = "NIfTI + Label Descriptor"
        ...
        public var fileExtensions: [String] {
            ...
            case .segmentationNRRD: return ["seg.nrrd"]
            case .dicomSeg:      return ["dcm"]
            case .dicomRTStruct: return ["dcm"]
            case .labelDescriptor: return ["nii", "nii.gz"]
    ...
    public static func saveNIfTI(_ label: LabelMap,
        ...
        if writeLabelDescriptor {
            try saveLabelDescriptor(label, to: url.deletingPathExtension().appendingPathExtension("label.txt"))
        }
    }
    ...
    public static func loadNIfTILabelmap(from url: URL,
        ...
        // Try to load a matching label descriptor.
        ...
        if FileManager.default.fileExists(atPath: descURL.path) {
            if let classes = try? loadLabelDescriptor(from: descURL) {
    ...
    // MARK: - Label descriptor (.label.txt)

    /// Save a label descriptor:
    ///     IDX   R   G   B   A  VIS  MSH  LABEL
    public static func saveLabelDescriptor(_ label: LabelMap, to url: URL) throws {
        var txt = "################################################\n"
        txt += "# Label Description File\n"
    ...
    /// Load a label descriptor.
    public static func loadLabelDescriptor(from url: URL) throws -> [LabelClass] {
    ...
    // MARK: - Segmentation NRRD (.seg.nrrd)

    /// Save a segmentation NRRD with segment metadata.
    public static func saveSegmentationNRRD(_ label: LabelMap,
                                            to url: URL,
                                            parentVolume: ImageVolume) throws {
        var header = "NRRD0004\n"
        header += "# Generated by Tracer\n"
    ...
    private static func classesFromNRRD(fields: [String: String], voxels: [UInt16]) -> [LabelClass] {
        ...
        for segmentIndex in uniqueSegmentIndices {
            ...
            let color = colorFromNRRDSegmentString(fields["\(prefix)Color"]) ?? autogeneratedColor(index: classes.count)
            classes.append(LabelClass(
                labelID: labelValue,
                name: name,
    ...
    private static func colorFromNRRDSegmentString(_ value: String?) -> Color? {
